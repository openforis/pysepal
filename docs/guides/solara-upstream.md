...
### Documented — `FileBrowserMultiple` (1.62)
...
so an app that wants it pins Solara itself. pysepal does not depend on it.
...
@solara.component
def RasterPicker(
    selected: solara.Reactive[list[Path]],
    directory: Path = Path.home(),
):
    solara.FileBrowserMultiple(
        directory=directory,
        selected=selected,
        filter=lambda path: path.is_dir() or path.suffix.lower() in {".tif", ".tiff"},
        directory_first=True,
    )
    solara.Text(msg("picker.count", count=len(selected.value)))
```

Selection lives in a reactive the caller owns, the `filter` keeps directories
traversable while narrowing files, and the label goes through `msg()` with a
plural node — the same rules as any other pysepal component.
